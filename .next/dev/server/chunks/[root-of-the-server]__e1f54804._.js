...
"[project]/Rank Time (Vibe)/ranktime/src/app/api/user/stats/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(request) {
    try {
        ...
        const recentSessions = await __TURBOPACK__imported__module__$5b$project$5d2f$Rank__Time__$28$Vibe$292f$ranktime$2f$src$2f$models$2f$Session$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["default"].find({
            userId: user._id
        }).sort({
            createdAt: -1
        }).limit(10);
        // Calculate additional stats
        const allSessions = await __TURBOPACK__imported__module__$5b$project$5d2f$Rank__Time__$28$Vibe$292f$ranktime$2f$src$2f$models$2f$Session$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["default"].find({
            userId: user._id
        });
        const totalTime = allSessions.reduce((sum, session)=>sum + session.totalTime, 0);
        const averageScore = allSessions.length > 0 ? allSessions.reduce((sum, session)=>sum + session.score, 0) / allSessions.length : 0;
        const averageRating = allSessions.length > 0 ? allSessions.reduce((sum, session)=>sum + session.problemRating, 0) / allSessions.length : 0;
        ...
        const nextThreshold = nextLeague ? leagueThresholds[nextLeague] : null;
        const currentThreshold = leagueThresholds[user.league] || 0;
        const leagueProgress = nextThreshold ? (user.totalScore - currentThreshold) / (nextThreshold - currentThreshold) * 100 : 100;
        const stats = {
            user: {
                name: user.name,
                email: user.email,
        ...
        return __TURBOPACK__imported__module__$5b$project$5d2f$Rank__Time__$28$Vibe$292f$ranktime$2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json(stats);
    } catch (error) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__e1f54804._.js.map